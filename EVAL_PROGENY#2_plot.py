...
groups_stage = ["healthy","pTa low", "pTa high", "pTis", "pT1a/b/G1/G2", "pT1c/G3", "pT2", ">pT2","pN1"]
...
vmax = 2.
data.T.to_excel("*PAULA/PROGENY/*PLOT_values_binary.xlsx")
...
data.T.to_excel("*PAULA/PROGENY/*PLOT_values_pTNM.xlsx")
#data = np.log(data.div(data.loc[0,:],axis="columns"))
#vmax = data.max().max()#np.max([np.abs(data.min().min()), data.max().max()])
print vmax
ax.imshow(data.T,cmap="RdBu_r",aspect="auto", vmin=-vmax, vmax=vmax)
...
data.T.to_excel("*PAULA/PROGENY/*PLOT_values_PAULA.xlsx")
#data = np.log(data.div(data.loc[0,:],axis="columns"))
#vmax = data.max().max()#np.max([np.abs(data.min().min()), data.max().max()])
print vmax
ax.imshow(data.T,cmap="RdBu_r",aspect="auto", vmin=-vmax, vmax=vmax)
