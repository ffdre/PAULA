...
for Group1 in range(1,9):
    ...
    for j, group1 in enumerate([group_list[Group1]]):
        for k, group2 in enumerate([group_list[Group2]]):
            k += (j+1)
            ...
            res.to_excel(data_folder + "/SPECIFICITY/RES_expressionDE_pTNM_%s-vs-%s.xlsx"%(group1, group2), index=False)
    ...
    #plt.show()
    fig.savefig(data_folder + "/SPECIFICITY/PLOT_expressionDE_pTNM_%s-vs-%s.png"%(group1, group2),dpi=300, transparent=True)