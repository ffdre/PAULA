...
group_list = []
for group in data_clinical.loc[:, group_column].unique():
    groups[group] = []
...
for j, group1 in enumerate(group_list[Group1:Group1+1]):
    for k, group2 in enumerate(group_list[Group2:Group2+1]):
        k += (j+1)
        ...
        res.to_excel(data_folder + "/NORMICS_results/RES_expressionDE_%s_%s-vs-%s.xlsx"%(mode, group1, group2), index=False)
...
plt.show()
fig.savefig(data_folder + "/NORMICS_figures/PLOT_PUBL_volcano_%s_%s.png"%(data_folder, mode),dpi=300, transparent=True)