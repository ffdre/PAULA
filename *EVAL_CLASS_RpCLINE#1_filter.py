...
try: os.mkdir(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome")
except: pass
...
if filter_CCL == True:
    CCL = pd.read_excel(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome/data_clines.xlsx").iloc[:,:]
    ...
    CCL.to_excel(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome/CCL_data_filtered.xlsx", index=False)

if filter_corr == True:
    if filter_CCL == False:
        CCL = pd.read_excel(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome/CCL_data_filtered.xlsx")
    data_corr = pd.read_excel(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome/*RES_data-proteomics-celllines-subset_correlations.xlsx")
    for i,row in CCL.iterrows():
        try:
            ...
            CCL = CCL.drop(labels=[i], axis=0)
    CCL.to_excel(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome/CCL_data_filtered_positively-correlated.xlsx", index=False)
else:
    CCL = pd.read_excel(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome/CCL_data_filtered_positively-correlated.xlsx")
...
CCL[["Accession"]+list(sorted_samples)].to_excel(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome/RES_CCL_binary.xlsx", index=False)
...
data[["Accession"]+sorted_samples].append(pd.DataFrame([["Cluster"]+list(data_clinical["cluster k=5"])], \
                                                       columns=data.columns)).to_excel(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome/RES_binary.xlsx", index=False)
...
fig.savefig(data_folder + "/CLASSIFIER/CLINES_RNA_by_proteome/PLOT_expression-heatmap.png", dpi=300)
plt.show()